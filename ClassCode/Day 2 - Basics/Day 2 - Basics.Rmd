...
Today we'll explore the gapminder data. It contains life expectancy, population, and GDP per capita for countries every five years from 1952 to 2007. We'll continue our exploration using R to understand and visualize data.
...
Country and continent are not continuous(numeric in R speak) variables. They are categorical and we can get counts in each category.
...
```{r}
notrich <- gapminder[which(gapminder$gdpPercap< 50000),] # less than 50,000 - this drops oil-rich outliers like Kuwait
```

Check in: Try creating a scatterplot with just the 1) Afrida OR 2) country-years where life expectancy is less than 70.
...
#Looking at the data over time: Time to look at something a bit more fun. How has life expectancy changed across the world since the 1950s? The gapminder data comes from the Gapminder Foundation: <https://www.gapminder.org/data/> made famous by Hans Rosling's TED talks.



As usual, look at the first and last lines of the data frame
...
Number of rows in your data? What is our unit of analysis? (Hint: it is not countries - each country shows up once for every year)
...
summary(gapminder) # Runs for all numeric variables in the dataset
```
...
Range of life expectancies in this data

```{r}
range(gapminder$lifeExp) 
```

Check: What is the average population(pop)
...
abline(v=mean(gapminder$lifeExp), col="blue", lwd=2)
abline(v=median(gapminder$lifeExp), col="red", lwd=2)
```
...
```{r}
g2007 <- gapminder[which(gapminder$year==2007),]

#New Graph
plot(g2007$gdpPercap, g2007$lifeExp, ylab = "Life Expectancy", xlab = "GDP per Capita", main = "Wealth and Life Expectancy in 2007", col=factor(g2007$continent))
...
g2007$stars <- ifelse(g2007$country=="United States"|g2007$country=="China", as.character(g2007$country), "") #create a new variable for notable countries, all else will be empty. 
text(g2007$gdpPercap, g2007$lifeExp-1, g2007$stars, cex=.4)
```
